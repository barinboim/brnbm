...
function signatureUrl(name) {
  return `${SIGNATURES_DIR}/${encodeURIComponent(name)}?t=${CACHE_BUSTER}`;
}
...
async function loadSignatures() {
  const response = await fetch(`${SIGNATURES_DIR}/manifest.json?t=${CACHE_BUSTER}`, {
    cache: "no-store",
  });
  if (!response.ok) throw new Error("manifest.json not found");
  const items = await response.json();
  state.signatures = items.map((item) => ({
    name: item.name,
    label: item.label ?? item.name.replace(/\.png$/i, ""),
    url: signatureUrl(item.name),
    width: 1,
    height: 1,
  }));
...
async function downloadSignedPdf() {
  ...
  for (const placement of state.placements) {
    ...
    const bytes = await fetch(signatureUrl(placement.signature)).then((r) => r.arrayBuffer());
    embedCache.set(placement.signature, await pdfDoc.embedPng(bytes));
  }
