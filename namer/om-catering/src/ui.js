...
(function () {
  ...
  const state = {
    ...
    langFilter: null, // Set включённых кодов языков (задаётся при загрузке проекта)
  };
  ...
  // Честный код языка слова: RU / EN / FR / IT / LAT / …
  function langBadge(word) {
    return (word.lang || "ru").toUpperCase();
  }
  function initPoints() { state.points = currentProject().matrices.map(() => ({ x: 0, y: 0 })); }

  // ——— Фильтр по языкам ———
  const LANG_PRIORITY = ["ru", "en", "fr", "it", "es", "pt", "de", "ro", "fi", "tr", "lat", "el", "ja", "sa"];
  function projectLangs() {
    const c = {};
    currentProject().words.forEach((w) => (c[w.lang] = (c[w.lang] || 0) + 1));
  ...
  function initLangFilter() { state.langFilter = new Set(projectLangs().map((x) => x.lang)); }
  // Проект с отфильтрованным по языкам набором слов (для ранжирования).
  function filteredProject() {
    const p = currentProject();
    if (!state.langFilter) return p;
    return { name: p.name, anchor: p.anchor, matrices: p.matrices, words: p.words.filter((w) => state.langFilter.has(w.lang)) };
  }
  ...
  UI.mount = function (root) {
    ...
    padsHead.appendChild(el("span", "section-meta", "Обе влияют на выборку"));
    ...
    nodes.langFilter = el("div", "lang-filter");
    nodes.namesView.appendChild(nodes.langFilter);
    ...
    // Перепроверяем кегль локапов при изменении ширины окна.
    window.addEventListener("resize", () => fitLockups(nodes.results));
  };
  ...
  // ——— Чипы-переключатели языков ———
  function buildLangFilter() {
    if (!nodes.langFilter) return;
    nodes.langFilter.innerHTML = "";
    const langs = projectLangs();
    langs.forEach(({ lang, count }) => {
      const on = state.langFilter.has(lang);
      ...
      chip.addEventListener("click", () => {
        if (state.langFilter.has(lang)) state.langFilter.delete(lang);
        else state.langFilter.add(lang);
        buildLangFilter();
        renderResults();
      });
      nodes.langFilter.appendChild(chip);
    });
    const all = el("button", "lang-chip lang-chip-act", "Все");
    all.addEventListener("click", () => { state.langFilter = new Set(langs.map((x) => x.lang)); buildLangFilter(); renderResults(); });
    const none = el("button", "lang-chip lang-chip-act", "Снять");
    none.addEventListener("click", () => { state.langFilter = new Set(); buildLangFilter(); renderResults(); });
    nodes.langFilter.appendChild(all);
    nodes.langFilter.appendChild(none);
  }
  ...
  function lockup(brand, anchorWord) {
    ...
    // Русские слова не имеют латинской формы (транслитерация не нужна) — пустая ячейка.
    if (!brand) { wrap.classList.add("lockup-empty"); return wrap; }
  ...
  // Динамически ужимаем кегль локапа, если нейм (особенно словосочетание) не влезает в колонку.
  function fitLockups(container) {
    const cells = container.querySelectorAll(".name-cell");
    cells.forEach((cell) => {
      const lk = cell.querySelector(".lockup");
      if (!lk || !lk.firstChild) return;
      lk.style.fontSize = "";
      const avail = cell.clientWidth;
      if (avail <= 0) return;
      const natural = lk.scrollWidth;
      if (natural > avail) {
        const size = Math.max(11, Math.floor((18 * avail) / natural));
        lk.style.fontSize = size + "px";
      }
    });
  }
  ...
  function renderResults() {
    ...
    const total = currentProject().words.length;
    nodes.resultCount.textContent =
      project.words.length === total ? total + " слов в базе" : project.words.length + " из " + total + " слов";

    if (!ranked.length) {
      nodes.results.innerHTML = "";
      nodes.results.appendChild(el("div", "empty", "Все языки выключены — включите хотя бы один."));
      return;
    }
    ...
    ranked.forEach((r) => {
      ...
      // Кириллица — слева (основная колонка)
      ...
      // Чип = перевод/значение слова (а не имя полюса).
      if (word.gloss) meta.appendChild(el("span", "tag-chip", word.gloss));
  ...
  function renderShortlist() {
    ...
    list.forEach((cyr) => {
      const word = byCyr[cyr] || { cyr: cyr, lat: "" };
